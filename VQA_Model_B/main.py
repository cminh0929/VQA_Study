...
CONFIG = {
    ...
    # --- Dataset ---
    'dataset': 'vqa2_full',      # using the fully cleaned VQA animals real dataset
...
def cmd_compare():
    ...
    print(f"\n{'='*60}")
    print("  PER-CATEGORY ACCURACY (Top 5 Categories)")
    print(f"{'='*60}\n")
    
    # Extract dynamic categories from the first available model that has per_category data
    all_categories = set()
    for m in results.values():
        if 'per_category' in m:
            all_categories.update(m['per_category'].keys())
            
    # Sort categories by frequency if possible or just take first 5
    # Since we don't have global counts easily, we'll just sort alphabetically and pick up to 5
    categories = sorted(list(all_categories))[:5]
